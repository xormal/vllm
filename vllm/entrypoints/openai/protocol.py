...
from openai.types.responses import (
    ...
    ResponseQueuedEvent as OpenAIResponseQueuedEvent,
)
...
except ImportError:  # Older OpenAI python clients (<1.65) use ImageGen prefix.
...
class OpenAIErrorType(str, Enum):
    """OpenAI-compatible error type identifiers."""
    ...
    BAD_REQUEST_ERROR = "BadRequestError"  # Backward compatibility


class ErrorInfo(OpenAIBaseModel):
    message: str
    type: OpenAIErrorType | str
    param: str | None = None
    code: int
...
class ResponsesRequest(OpenAIBaseModel):
    ...
    prompt_cache_key: str | None = Field(
        default=None,
        description="OpenAI prompt cache key used to isolate prefix cache entries.",
    )
    ...
    def to_sampling_params(
        ...
        if (repetition_penalty := self.repetition_penalty) is None:
            repetition_penalty = default_sampling_params.get(
                "repetition_penalty", self._DEFAULT_SAMPLING_PARAMS["repetition_penalty"]
            )
        ...
        # Handle seed with randomization
        if self.seed is not None:
            # Explicit seed always takes priority
            seed = self.seed
        elif self.randomize:
            # Generate random seed if randomize=True and no explicit seed
            seed = random.randint(0, 2**32 - 1)
        else:
            # randomize=False and no explicit seed -> deterministic (None)
            seed = None
    ...
    @model_validator(mode="before")
    def validate_prompt_cache_key(cls, data):
        key = data.get("prompt_cache_key")
        if key is not None:
            if not isinstance(key, str) or not key:
                raise ValueError(
                    "Parameter 'prompt_cache_key' must be a non-empty string if provided."
                )
        return data
            ...
            if isinstance(item, dict):
                ...
                if item_type == "function_call":
                    ...
                    if not normalized_item.get("id"):
                        normalized_item["id"] = f"rs_{random_uuid()}"
                    if "status" not in normalized_item:
                        normalized_item["status"] = None
                    try:
                    ...
                    normalized_item = dict(item)
                    if not normalized_item.get("id"):
                        normalized_item["id"] = f"msg_{random_uuid()}"
                    normalized_item.setdefault("status", "completed")
...
class ResponsesToolOutputItem(OpenAIBaseModel):
    """Individual output payload supplied to /responses/{id}/tool_outputs."""

    type: Literal["output_text"] = "output_text"
    text: str


class ResponsesToolOutputsRequest(OpenAIBaseModel):
    tool_call_id: str
    output: list[ResponsesToolOutputItem]


class ConversationItem(OpenAIBaseModel):
    id: str
...
class ConversationItemsCreateRequest(OpenAIBaseModel):
    items: list[ConversationItemCreate]
...
class ConversationCreateRequest(OpenAIBaseModel):
    title: str | None = None
    metadata: dict[str, Any] | None = None
    status: Literal["active", "archived"] | None = None
    items: list[ConversationItemCreate] | None = None


class ConversationUpdateRequest(OpenAIBaseModel):
    title: str | None = None
    metadata: dict[str, Any] | None = None
    status: Literal["active", "archived"] | None = None


class ConversationListItemsResponse(OpenAIBaseModel):
...
class ResponseToolCallCompletedEvent(OpenAIBaseModel):
    """Event emitted when a tool call's argument stream has completed."""
...
class ResponseReasoningDeltaEvent(OpenAIBaseModel):
    ...
    response: dict[str, Any] = Field(
        ...,
        description="Minimal response metadata (typically {'id': response_id}).",
    )
    delta: str = Field(
        ...,
...
class ResponseReasoningDoneEvent(OpenAIBaseModel):
    """OpenAI-compatible reasoning.done terminal event."""
    ...
    reasoning: dict[str, str] = Field(
        ...,
        description="Final reasoning payload.",
        examples=[{"content": "Complete reasoning transcript."}],
    )
    sequence_number: int
...
class ResponseReasoningSummaryAddedEvent(OpenAIBaseModel):
    """Event emitted when reasoning summary streaming begins."""
...
class ResponseReasoningSummaryPart(OpenAIBaseModel):
    """Summary part payload emitted with reasoning summary events."""

    type: Literal["summary_text"] = "summary_text"
    text: str = ""


class ResponseReasoningSummaryPartAddedEvent(OpenAIBaseModel):
    """Event emitted when a reasoning summary part begins streaming."""
...
class ResponseReasoningSummaryPartDoneEvent(OpenAIBaseModel):
    """Event emitted when a reasoning summary part finishes streaming."""
...
class ResponseAdditionalContextEvent(OpenAIBaseModel):
    """Event that carries encrypted reasoning or other context."""

    type: Literal["response.additional_context"] = "response.additional_context"
    response: dict[str, Any]
    context: dict[str, Any] = Field(
        ...,
        description="Additional context values keyed by include target.",
    )
    sequence_number: int
...
class RateLimitInfo(OpenAIBaseModel):
    ...
    used_percent: float = Field(
        ..., description="Percentage of limit used (0-100).", ge=0.0, le=100.0
    )
    window_minutes: int = Field(..., description="Window size for this limit.")
    limit_type: Literal["primary", "requests", "tokens", "images"] = "primary"


class ResponseRateLimitsUpdatedEvent(OpenAIBaseModel):
    """Event emitted when rate limit usage is updated."""
...
class ResponseErrorEvent(OpenAIBaseModel):
    """Event emitted when a streaming response fails."""
...
class ResponsePingEvent(OpenAIBaseModel):
    """Heartbeat event emitted to keep SSE connections alive."""

    type: Literal["response.ping"] = "response.ping"
...
class ResponseQueuedEvent(OpenAIResponseQueuedEvent):
    response: ResponsesResponse  # type: ignore[override]
