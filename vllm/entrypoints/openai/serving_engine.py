...
class OpenAIServing:
    ...
    def create_error_response(
        ...
        normalized_type = self._normalize_error_type(err_type, status_code)
        normalized_status = self._normalize_status_code(status_code, normalized_type)
    ...
    def _normalize_error_type(
        self,
        err_type: OpenAIErrorType | str | None,
        status_code: HTTPStatus | int | None,
    ) -> OpenAIErrorType | str:
        if err_type is None and status_code is not None:
            return self._error_type_from_status(int(status_code))
        if err_type is None:
            return OpenAIErrorType.INVALID_REQUEST_ERROR
        if isinstance(err_type, OpenAIErrorType):
            return err_type
        try:
            return OpenAIErrorType(err_type)
        except ValueError:
            return err_type
    ...
    def _normalize_status_code(
        self,
        status_code: HTTPStatus | int | None,
        err_type: OpenAIErrorType | str,
    ) -> int:
        if status_code is not None:
            return int(status_code)

        enum_type: OpenAIErrorType | None = None
        if isinstance(err_type, OpenAIErrorType):
            enum_type = err_type
        else:
            try:
                enum_type = OpenAIErrorType(err_type)
            except ValueError:
                return HTTPStatus.BAD_REQUEST.value
        return self._status_from_error_type(enum_type)

    @staticmethod
    def _error_type_from_status(status_code: int) -> OpenAIErrorType:
        mapping: dict[int, OpenAIErrorType] = {
            HTTPStatus.BAD_REQUEST.value: OpenAIErrorType.INVALID_REQUEST_ERROR,
            HTTPStatus.UNAUTHORIZED.value: OpenAIErrorType.AUTHENTICATION_ERROR,
            HTTPStatus.FORBIDDEN.value: OpenAIErrorType.AUTHENTICATION_ERROR,
            HTTPStatus.NOT_FOUND.value: OpenAIErrorType.NOT_FOUND_ERROR,
            HTTPStatus.TOO_MANY_REQUESTS.value: OpenAIErrorType.RATE_LIMIT_ERROR,
            HTTPStatus.INTERNAL_SERVER_ERROR.value: OpenAIErrorType.INTERNAL_ERROR,
            HTTPStatus.BAD_GATEWAY.value: OpenAIErrorType.SERVICE_UNAVAILABLE,
            HTTPStatus.SERVICE_UNAVAILABLE.value: OpenAIErrorType.SERVICE_UNAVAILABLE,
            HTTPStatus.GATEWAY_TIMEOUT.value: OpenAIErrorType.TIMEOUT_ERROR,
        }
        return mapping.get(status_code, OpenAIErrorType.INTERNAL_ERROR)

    @staticmethod
    def _status_from_error_type(err_type: OpenAIErrorType) -> int:
        mapping: dict[OpenAIErrorType, int] = {
            OpenAIErrorType.INVALID_API_KEY: HTTPStatus.UNAUTHORIZED.value,
            OpenAIErrorType.AUTHENTICATION_ERROR: HTTPStatus.UNAUTHORIZED.value,
            OpenAIErrorType.INVALID_REQUEST_ERROR: HTTPStatus.BAD_REQUEST.value,
            OpenAIErrorType.INVALID_PARAMETER: HTTPStatus.BAD_REQUEST.value,
            OpenAIErrorType.NOT_FOUND_ERROR: HTTPStatus.NOT_FOUND.value,
            OpenAIErrorType.MODEL_NOT_FOUND: HTTPStatus.NOT_FOUND.value,
            OpenAIErrorType.RATE_LIMIT_ERROR: HTTPStatus.TOO_MANY_REQUESTS.value,
            OpenAIErrorType.TOKENS_LIMIT_REACHED: HTTPStatus.TOO_MANY_REQUESTS.value,
            OpenAIErrorType.REQUESTS_LIMIT_REACHED: HTTPStatus.TOO_MANY_REQUESTS.value,
            OpenAIErrorType.QUOTA_EXCEEDED: HTTPStatus.TOO_MANY_REQUESTS.value,
            OpenAIErrorType.INSUFFICIENT_QUOTA: HTTPStatus.TOO_MANY_REQUESTS.value,
            OpenAIErrorType.USAGE_LIMIT_REACHED: HTTPStatus.TOO_MANY_REQUESTS.value,
            OpenAIErrorType.USAGE_NOT_INCLUDED: HTTPStatus.TOO_MANY_REQUESTS.value,
            OpenAIErrorType.INTERNAL_ERROR: HTTPStatus.INTERNAL_SERVER_ERROR.value,
            OpenAIErrorType.INTERNAL_SERVER_ERROR: HTTPStatus.INTERNAL_SERVER_ERROR.value,
            OpenAIErrorType.SERVICE_UNAVAILABLE: HTTPStatus.SERVICE_UNAVAILABLE.value,
            OpenAIErrorType.TIMEOUT_ERROR: HTTPStatus.GATEWAY_TIMEOUT.value,
            OpenAIErrorType.CONTENT_FILTER: HTTPStatus.BAD_REQUEST.value,
            OpenAIErrorType.CONTENT_POLICY_VIOLATION: HTTPStatus.BAD_REQUEST.value,
            OpenAIErrorType.BAD_REQUEST: HTTPStatus.BAD_REQUEST.value,
            OpenAIErrorType.BAD_REQUEST_ERROR: HTTPStatus.BAD_REQUEST.value,
        }
        return mapping.get(err_type, HTTPStatus.INTERNAL_SERVER_ERROR.value)
    ...
    async def _check_model(
        ...
        return error_response or self.create_error_response(
            ...
            err_type=OpenAIErrorType.NOT_FOUND_ERROR,
    ...
    async def _generate_with_builtin_tools(
        ...
        **kwargs,
        ...
        while True:
            ...
            # Log sampling params being sent to engine
            logger.debug(
                "[ENGINE CALL] Request %s calling engine.generate() with sampling_params: "
                "temperature=%.2f, top_p=%.2f, top_k=%d, min_p=%.3f, "
                "repetition_penalty=%.2f, presence_penalty=%.2f, frequency_penalty=%.2f, "
                "seed=%s, max_tokens=%d",
                request_id,
                ...
                sampling_params.seed if sampling_params.seed is not None else "None",
