    ...
    async def load_lora_adapter(
        ...
        async with self.lora_resolver_lock[lora_name]:
            ...
            try:
                ...
                error_type: OpenAIErrorType = OpenAIErrorType.INVALID_REQUEST_ERROR
                status_code = HTTPStatus.BAD_REQUEST
                if "No adapter found" in str(e):
                    error_type = OpenAIErrorType.NOT_FOUND_ERROR
    ...
    async def _check_load_lora_adapter_request(
        ...
        if not request.lora_name or not request.lora_path:
            return create_error_response(
                ...
                err_type=OpenAIErrorType.INVALID_REQUEST_ERROR,
                status_code=HTTPStatus.BAD_REQUEST,
            )
...
def create_error_response(
    message: str,
    err_type: OpenAIErrorType | str = OpenAIErrorType.INVALID_REQUEST_ERROR,
    status_code: HTTPStatus = HTTPStatus.BAD_REQUEST,
) -> ErrorResponse:
    normalized_type: OpenAIErrorType | str
    if isinstance(err_type, OpenAIErrorType):
        normalized_type = err_type
    else:
        try:
            normalized_type = OpenAIErrorType(err_type)
        except ValueError:
            normalized_type = err_type

    return ErrorResponse(
        error=ErrorInfo(
            message=message, type=normalized_type, code=status_code.value
        )
    )
