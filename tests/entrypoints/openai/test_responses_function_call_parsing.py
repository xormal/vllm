...
def test_reasoning_item_without_id_is_normalized():
    """Ensure reasoning items missing ids are accepted by generating ids."""
    ...
    request_data = {
        ...
        "input": [
            ...
            },
            {
                "type": "reasoning",
                "summary": [],
                "content": [{"type": "reasoning_text", "text": "Working..."}],
            },
        ],
    }
    ...
    item_id = (
        ...
        else getattr(reasoning_item, "id", None)
    )
...
def test_assistant_message_missing_metadata_is_normalized():
    """Ensure assistant messages get default id/status/annotations."""
    ...
    request = ResponsesRequest(**request_data)
    assert len(request.input) == 1
    assistant_msg = request.input[0]

    assert assistant_msg["id"].startswith("msg_")
    assert assistant_msg["status"] == "completed"
    assert assistant_msg["content"][0]["annotations"] == []
